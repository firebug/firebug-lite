...
(function(window) {
    ...
    runner.prototype.runPage = function() {
        var doc = document;
        // update page counter
        doc.getElementById("qunit-runner-userAgent").innerHTML ='<span class="runner-test-page-counter">Test Page '
        ...
        var container = doc.getElementById("runner-test-page-container");

        var header = doc.createElement("p");
        header.className = "runner-test-page-header passed";
        header.innerHTML = test.title;
        header.isOpen = false;
    ...
    runner.prototype.pageProgress = function(frame, failures, total, testName, isDone) {
        ...
        for(var i=0, l=tests.length; i<l; i++)
        {
            ...
            if (test.frame === frame) {
                ...
                header.innerHTML = test.title + " " + testName
    ...
    if (top.__qunit_runner) {
        ...
        QUnit.done = function(test) {
            runner.pageProgress(window.frameElement, test.failed, test.total, "done", true);
        };
...
(function(window) {
    ...
    window.FBTest =
    {
        ...
        runTest: function(testCase)
        {
            ...
            document.documentElement.id = "runner-test-page";
        ...
        runTestOnLoad: function(testCase)
        {
            ...
            test("Loading Firebug Lite at " + fileName, function(){

                stop();

                function initializeTest()
                {
                    test("Testing " + fileName, testCase);
                    start();
                }

                if (FBTest.isFirebugInitialized())
                {
        ...
        focus: function(node)
        {
            ...
            {
                QUnit.triggerEvent(node, "focus");
            }
        },

        mouseDown: function(node)
        {
            QUnit.triggerEvent(node, "mousedown");
        },
        ...
        {
            return Firebug.chrome.getPanel(panelName);
        },

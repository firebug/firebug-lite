 ...
 */
var FBL={};
(function(){var namespaces=[];
var FBTrace=null;
this.ns=function(fn){var ns={};
namespaces.push(fn,ns);
return ns
};
this.initialize=function(){FBTrace=FBL.FBTrace;
if(FBL.Application.isDebugMode){FBTrace.initialize()
}var isChromeContext=FBL.Application.isPersistentMode&&typeof window.FirebugApplication=="object";
...
this.Application={isBookmarletMode:true,isPersistentMode:false,isDebugMode:true,skin:"xp",isDevelopmentMode:false,isChromeContext:false,browser:null,chrome:null};
var destroyApplication=function destroyApplication(){setTimeout(function(){FBL=null
},100)
};
this.Application.location={source:null,base:null,skin:null,app:null};
var findLocation=function findLocation(){var reFirebugFile=/(firebug(?:\.\w+)?\.js(?:\.gz)?)(#.+)?$/;
var rePath=/^(.*\/)/;
var reProtocol=/^\w+:\/\//;
var path=null;
for(var i=0,c=document.getElementsByTagName("script"),ci;
ci=c[i];
i++){var file=null;
if(ci.nodeName.toLowerCase()=="script"&&(file=reFirebugFile.exec(ci.src))){var fileName=file[1];
var fileOptions=file[2];
if(reProtocol.test(ci.src)){path=rePath.exec(ci.src)[1]
}else{var r=rePath.exec(ci.src);
...
if(path&&m){var loc=FBL.Application.location;
loc.source=path;
...
if(fileName=="firebug.dev.js"){FBL.Application.isDevelopmentMode=true
}if(fileOptions){}}else{throw new Error("Firebug Error: Library path not found")
}};
...
}this.escapeHTML=function(value){return(value+"").replace(/[<>&"']/g,replaceChars)
};
var reTrim=/^\s+|\s+$/g;
...
this.hasClass=function(object,name){return(" "+object.className+" ").indexOf(" "+name+" ")!=-1
};
this.addClass=function(object,name){if((" "+object.className+" ").indexOf(" "+name+" ")==-1){object.className=object.className?object.className+" "+name:name
}};
this.removeClass=function(object,name){object.className=(" "+object.className+" ").replace(new RegExp("(\\S*)\\s+"+name+"\\s+(\\S*)","g"),"$1 $2").replace(/^\s*|\s*$/g,"")
};
this.toggleClass=function(object,name){if((" "+object.className+" ").indexOf(" "+name+" ")>=0){this.removeClass(object,name)
}else{this.addClass(object,name)
}};
...
var panelTypes=[];
var panelTypeMap={};
FBL.Firebug={version:"Firebug Lite 1.3.0a",revision:"$Revision: 3728 $",modules:modules,panelTypes:panelTypes,chromeMap:{},initializeContext:function(chrome){},initialize:function(){if(FBTrace.DBG_INITIALIZE){FBTrace.sysout("Firebug.initialize","initializing application")
...
}}},cacheDocument:function(){var els=Firebug.browser.document.getElementsByTagName("*");
...
var tabNode=this.tabNode=createElement("a",{id:panelId+"Tab",className:"fbTab",innerHTML:tabHTML});
...
},changeDisplay:function(display){if(display!=this.display){if(display=="pressed"){addClass(this.node,"fbBtnPressed")
}else{if(display=="unpressed"){removeClass(this.node,"fbBtnPressed")
}}this.display=display
}},handlePress:function(){if(this.type=="normal"){this.changeDisplay("pressed");
...
}}}}},handleUnpress:function(){if(this.beforeClick){this.changeDisplay("unpressed")
}},handleClick:function(){if(this.type=="normal"){if(this.onClick){this.onClick.apply(this.owner)
...
var uidString=uid?[cacheID,'="',uid,'" id="',uid,'"'].join(""):"";
html.push('<div class="objectBox-element"',uidString,'">','&lt;<span class="nodeTag">',node.nodeName.toLowerCase(),"</span>");
for(var i=0;
...
var top=0;
if(el.offsetParent){do{left+=el.offsetLeft;
top+=el.offsetTop
...
el.removeChild(div);
return value
},getCSS:this.isIE?function(el,name){return el.currentStyle[name]||el.style[name]||undefined
...
FBL.ns(function(){with(FBL){FBL.FirebugChrome={commandLineVisible:true,sidePanelVisible:false,sidePanelWidth:300,selectedPanel:"Console",height:250,isOpen:false,create:function(){createChrome({onLoad:onChromeLoad})
},initialize:function(){var chrome=Firebug.chrome=new Chrome(Application.chrome);
Firebug.chromeMap[chrome.type]=chrome
}};
var onPopupChromeLoad=function(chromeContext){FBTrace.sysout("onPopupChromeLoad","-------------------------");
...
if(isChromeFrame){ChromeMini.create(chrome)
...
if(Application.isDevelopmentMode){FBDev.loadChromeApplication(chrome)
}else{var doc=chrome.document;
var script=doc.createElement("script");
script.src=Application.location.app;
doc.getElementsByTagName("head")[0].appendChild(script)
}}else{setTimeout(function(){FBL.Firebug.initialize()
},100)
}};
...
var ChromeBase=extend(ChromeBase,{create:function(){addGlobalEvent("keydown",onPressF12);
...
}}},detach:function(){if(!Firebug.chromeMap.popup){createChrome({type:"popup",onLoad:onPopupChromeLoad})
...
if(isIE&&self.type=="frame"){self.fixIEPosition()
}},0)
...
}if(FirebugChrome.isOpen){this.open()
}else{FirebugChrome.isOpen=true;
this.close()
}if(this.node.style.visibility!="visible"){this.node.style.visibility="visible"
...
if(isIE6){this.addController([Firebug.browser.window,"scroll",this.fixIEPosition])
...
}},close:function(){if(FirebugChrome.isOpen){if(this.isInitialized){dispatch(Firebug.modules,"shutdown",[]);
this.shutdown()
}var main=$("fbChrome");
main.style.display="none";
FirebugChrome.isOpen=false;
ChromeMini.initialize()
}},fixIEPosition:function(){var offset=isIE?this.document.body.clientTop||this.document.documentElement.clientTop:0;
...
if(frameElement){var framePos=Firebug.Inspector.getElementPosition(frameElement).top;
clientY+=framePos;
...
if(isIE6){Firebug.chrome.fixIEPosition()
}Firebug.chrome.draw()
};
...
FBL.ns(function(){with(FBL){Firebug.Inspector={initialize:function(){offlineFragment=document.createDocumentFragment();
...
}for(var name in outline){document.body.appendChild(outlineElements[name])
}outlineVisible=true
...
},hideBoxModel:function(){offlineFragment.appendChild(boxModel);
boxModelVisible=false
},showBoxModel:function(){document.body.appendChild(boxModel);
...
}}},getElementPosition:function(el){var left=0,top=0;
...
}else{if((el=document.documentElement)&&(el.clientHeight||el.clientWidth)){width=el.clientWidth;
height=el.clientHeight
}else{if((el=document.body)&&(el.clientHeight||el.clientWidth)){width=el.clientWidth;
height=el.clientHeight
}}}return{width:width,height:height}
},getWindowScrollSize:function(){var width=0,height=0,el;
if(!isIEQuiksMode&&(el=document.documentElement)&&(el.scrollHeight||el.scrollWidth)){width=el.scrollWidth;
height=el.scrollHeight
}else{if((el=document.body)&&(el.scrollHeight||el.scrollWidth)){width=el.scrollWidth;
height=el.scrollHeight
}}return{width:width,height:height}
},getWindowScrollPosition:function(){var top=0,left=0,el;
if(typeof window.pageYOffset=="number"){top=window.pageYOffset;
left=window.pageXOffset
}else{if((el=document.body)&&(el.scrollTop||el.scrollLeft)){top=el.scrollTop;
left=el.scrollLeft
}else{if((el=document.documentElement)&&(el.scrollTop||el.scrollLeft)){top=el.scrollTop;
left=el.scrollLeft
}}}return{top:top,left:left}
},getElementBox:function(el){var result={};
if(el.getBoundingClientRect){var rect=el.getBoundingClientRect();
...
}else{return document.elementFromPoint(x,y)
}},getMeasurementBox:function(el,name){var sufixes=["Top","Left","Bottom","Right"];
var result=[];
for(var i=0,sufix;
sufix=sufixes[i];
...
if(size.unit=="px"){return size.value
}var computeDirtyFontSize=function(el,calibration){var div=document.createElement("div");
var divStyle=offscreenStyle;
if(calibration){divStyle+=" font-size:"+calibration+"px;"
}div.style.cssText=divStyle;
div.innerHTML="A";
...
}:function(el,name){return document.defaultView.getComputedStyle(el,null)[name]||el.style[name]||undefined
}};
...
var calculatePixelsPerInch=function calculatePixelsPerInch(){var inch=document.createElement("div");
inch.style.cssText=resetStyle+"width:1in; height:1in; position:absolute; top:-1234px; left:-1234px;";
document.body.appendChild(inch);
pixelsPerInch={x:inch.offsetWidth,y:inch.offsetHeight};
document.body.removeChild(inch)
};
var createInspectorFrame=function createInspectorFrame(){fbInspectFrame=document.createElement("div");
fbInspectFrame.id="fbInspectFrame";
fbInspectFrame.style.cssText=inspectFrameStyle;
document.body.appendChild(fbInspectFrame)
};
var destroyInspectorFrame=function createInspectorFrame(){document.body.removeChild(fbInspectFrame)
};
var createOutlineInspector=function createOutlineInspector(){for(var name in outline){var el=outlineElements[name]=document.createElement("div");
el.id=name;
el.style.cssText=inspectStyle+outlineStyle[outline[name]];
offlineFragment.appendChild(el)
}};
var createBoxModelInspector=function createBoxModelInspector(){boxModel=document.createElement("div");
...
},evaluate:function(expr){var api="FBL.Firebug.CommandLine.API";
...
}if(typeof uid!="undefined"){html.push('<div class="objectBox-element" ',cacheID,'="',uid,'" id="',uid,'">',!isIE&&nodeControl?nodeControl:"",'&lt;<span class="nodeTag">',nodeName,"</span>")
}else{html.push('<div class="objectBox-element">&lt;<span class="nodeTag">',nodeName,"</span>")
}for(var i=0;
i<node.attributes.length;
++i){var attr=node.attributes[i];
...
if(value){html.push('&gt;<span class="nodeText">',escapeHTML(value),'</span>&lt;/<span class="nodeTag">',nodeName,"</span>&gt;</div>")
}else{html.push("/&gt;</div>")
}}else{if(childLength>0){html.push("&gt;</div>")
}else{html.push("/&gt;</div>")
...
if(Firebug.chrome.type!="popup"){this.createUI()
...
}if(targ.className.indexOf("nodeControl")!=-1||targ.className=="nodeTag"){if(targ.className=="nodeTag"){var control=FBL.isIE?(targ.parentNode.previousSibling||targ):(targ.previousSibling.previousSibling||targ);
...
}if(targ.id=="fbConsole"){FBL.Firebug.Inspector.hideBoxModel();
hoverElement=null
...
while(targ&&!found){if(!/\sobjectBox-element\s|\sobjectBox-selector\s/.test(" "+targ.className+" ")){targ=targ.parentNode
}else{found=true
...
}if(" html head body br script link ".indexOf(" "+nodeName+" ")!=-1){FBL.Firebug.Inspector.hideBoxModel();
hoverElement=null;
return
...
FBL.ns(function(){with(FBL){if(!Application.isDebugMode){return

...
var FBTrace=null;
this.initialize=function(){FBL.Env.Cache={Chrome:null};
var isChromeContext=typeof window.FirebugApplication=="object";
if(!isChromeContext){findLocation()
}FBTrace=FBL.FBTrace;
if(isChromeContext){FBL.Env=window.FirebugApplication;
...
}if(FBL.Env.isPersistentMode){if(isChromeContext){FBL.Env.Cache.Chrome.clone(FBL.Env._lastChromeCache)
}else{FBL.Env._lastChromeCache=FBL.Env.Cache.Chrome;
...
if(!FBL.Env.isDevelopmentMode){window.FirebugApplication.destroy();
if(FBL.isIE){window.FirebugApplication=null
}else{delete window.FirebugApplication
}}}else{FBL.Env.Cache.Chrome.create()
}};
this.Env={openAtStartup:false,isBookmarletMode:false,isPersistentMode:false,isTraceMode:false,skin:"xp",isDevelopmentMode:false,isChromeContext:false,browser:null,chrome:null};
...
var parentPanelMap={};
var reps=[];
FBL.Firebug={version:"Firebug Lite 1.3.0a3",revision:"$Revision: 4979 $",modules:modules,panelTypes:panelTypes,reps:reps,initialize:function(){if(FBTrace.DBG_INITIALIZE){FBTrace.sysout("Firebug.initialize","initializing application")
...
Firebug.PanelBar={selectedPanel:null,isSidePanelBar:null,create:function(isSidePanelBar){this.panelMap={};
...
},addPanel:function(panelName,parentPanel){var PanelType=panelTypeMap[panelName];
...
selectedPanel.shutdown();
selectedPanel.hide()
}if(!panel.parentPanel){Env.Cache.Chrome.selectedPanel=panelName
...
var ChromeCache=FBL.Env.Cache.Chrome={commandLineVisible:false,sidePanelVisible:false,sidePanelWidth:300,selectedPanel:"Console",selectedElement:null,consoleMessageQueue:[],height:250,isOpen:false,create:function(){if(FBTrace.DBG_INITIALIZE){FBTrace.sysout("Env.Cache.Chrome.create","creating chrome window")
}createChrome()
},initialize:function(){if(FBTrace.DBG_INITIALIZE){FBTrace.sysout("Env.Cache.Chrome.initialize","initializing chrome window")
}if(Env.chrome.type=="frame"){ChromeMini.create(Env.chrome)
}if(Env.browser.document.documentElement.getAttribute("debug")=="true"){Env.openAtStartup=true
}var chrome=Firebug.chrome=new Chrome(Env.chrome);
chromeMap[chrome.type]=chrome;
addGlobalEvent("keydown",onPressF12);
if(Env.isPersistentMode&&chrome.type=="popup"){chromeMap.frame=ChromeCache.chromeMap.frame;
...
}else{var height=ChromeCache.height||options.height;
...
}if(Env.isPersistentMode){Env._lastChromeCache=ChromeCache;
...
var getChromeTemplate=function(isPopup){var tpl=ChromeCache.injected;
var r=[],i=-1;
...
var self=this;
setTimeout(function(){self.selectPanel(ChromeCache.selectedPanel)
},0)
...
var commandLineHeight=ChromeCache.commandLineVisible?fbCommandLine.offsetHeight:0,y=Math.max(size.height,topHeight),height=Math.max(y-topHeight-commandLineHeight,0)+"px",sideWidth=ChromeCache.sidePanelVisible?ChromeCache.sidePanelWidth:0,width=Math.max(size.width-sideWidth,0)+"px";
...
if(ChromeCache.sidePanelVisible){sideWidth=Math.max(sideWidth-6,0)+"px";
...
}if(Env.openAtStartup){this.open()
}else{ChromeCache.isOpen=true;
this.close()
...
},open:function(){if(!ChromeCache.isOpen){var node=this.node;
node.style.visibility="hidden";
...
}},close:function(){if(ChromeCache.isOpen){var node=this.node;
node.style.visibility="hidden";
...
if(Env.isPersistentMode){Env._lastChromeCache.selectedElement=ChromeCache.selectedElement
}frame.reattach(this,frame);
ChromeBase.destroy.apply(this);
if(Env.isPersistentMode){Env._lastChromeCache.chromeMap=ChromeCache.chromeMap;
...
var changeCommandLineVisibility=function changeCommandLineVisibility(visibility){var last=ChromeCache.commandLineVisible;
Firebug.chrome.commandLineVisible=ChromeCache.commandLineVisible=typeof visibility=="boolean"?visibility:!ChromeCache.commandLineVisible;
if(ChromeCache.commandLineVisible!=last){fbBottom.className=ChromeCache.commandLineVisible?"":"hide"
}};
var changeSidePanelVisibility=function changeSidePanelVisibility(visibility){var last=ChromeCache.sidePanelVisible;
Firebug.chrome.sidePanelVisible=ChromeCache.sidePanelVisible=typeof visibility=="boolean"?visibility:!ChromeCache.sidePanelVisible;
if(ChromeCache.sidePanelVisible!=last){fbPanelBox2.className=ChromeCache.sidePanelVisible?"":"hide";
...
if(isIE){addEvent(Env.browser.document.documentElement,"mouseleave",onHSplitterMouseUp)
...
if(isIE){removeEvent(Env.browser.document.documentElement,"mouseleave",onHSplitterMouseUp)
...
},flush:function(){var queue=Env.Cache.Chrome.consoleMessageQueue;
...
}else{Env.Cache.Chrome.consoleMessageQueue.push([message,className,handler])
}return this.LOG_COMMAND
...
if(!isFirefox){Env.browser.window.console=ConsoleAPI
}}});
...
FBL.ns(function(){with(FBL){Firebug.Inspector={create:function(){offlineFragment=Env.browser.document.createDocumentFragment();
...
var windowSize=Firebug.browser.getWindowSize();
var scrollPosition=Firebug.browser.getWindowScrollPosition();
var offsetHeight=Firebug.chrome.type=="frame"?Env.Cache.Chrome.height:0;
...
var self=this;
Env.browser.onerror=function(){self.onError.apply(self,arguments)
};
...
},reattach:function(){if(Env.Cache.Chrome.selectedElement){Firebug.HTML.selectTreeNode(Env.Cache.Chrome.selectedElement)
}}});
...
},renderStylesheet:function(index){var str=renderStylesheet(index);
var panel=this.getPanel();
panel.contentNode.innerHTML=str.join("");
panel.containerNode.scrollTop=0
}});
Firebug.registerModule(Firebug.CSS);
function CSSPanel(){}CSSPanel.prototype=extend(Firebug.Panel,{name:"CSS",title:"CSS",options:{hasToolButtons:true},create:function(){Firebug.Panel.create.apply(this,arguments);
...
i<len;
i++){var uri=getFileName(collection[i].href)||getFileName(doc.location.href);
...
function CSSPanel2(){}CSSPanel2.prototype=extend(Firebug.Panel,{name:"CSS2",parentPanel:"HTML",title:"CSS",options:{hasToolButtons:true},create:function(){Firebug.Panel.create.apply(this,arguments)
},initialize:function(){Firebug.Panel.initialize.apply(this,arguments);
...
for(var i=0,script;
script=scripts[i];
i++){var fileName=getFileName(script.src)||"..";
var option=createElement("option",{value:i});
...
this.renderSourceCode(index)
},renderSourceCode:function(index){if(this.lastSourceIndex!=index){var self=this;
var renderProcess=function renderProcess(src){var html=[],hl=0,s=[],sl=0;
...
function DOMPanel(){}DOMPanel.prototype=extend(Firebug.Panel,{name:"DOM",title:"DOM",options:{hasToolButtons:true},isInitialized:false,create:function(){Firebug.Panel.create.apply(this,arguments);
...
this.isInitialized=true
},reattach:function(oldChrome){this.toggles=oldChrome.getPanel("DOM").toggles
}});
Firebug.registerPanel(DOMPanel);
function DOMPanel2(){}DOMPanel2.prototype=extend(Firebug.Panel,{name:"DOM2",parentPanel:"HTML",title:"DOM",options:{hasToolButtons:true},isInitialized:false,create:function(){Firebug.Panel.create.apply(this,arguments);
this.toggles=this.toggles||{};
this.panelNode.style.padding="0 1px"
},initialize:function(){Firebug.Panel.initialize.apply(this,arguments);
var target=this.contentNode;

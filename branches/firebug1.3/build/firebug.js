...
this.collapse=function(elt,collapsed){if(this.isIE6){if(collapsed){this.setClass(elt,"collapsed")
...
this.selectInputRange=function(input,start,length){if(input.createTextRange){var range=input.createTextRange();
...
var reps=[];
var parentPanelMap={};
window.Firebug=FBL.Firebug={version:"Firebug Lite 1.3.0a6",revision:"$Revision: 5833 $",modules:modules,panelTypes:panelTypes,panelTypeMap:panelTypeMap,reps:reps,initialize:function(){if(FBTrace.DBG_INITIALIZE){FBTrace.sysout("Firebug.initialize","initializing application")
...
Firebug.chrome.draw();
if(fbVSplitterStyle){fbVSplitterStyle.right=FirebugChrome.sidePanelWidth-5+"px"
...
if(result!=Firebug.Console.LOG_COMMAND){var html=[];
...
Firebug.InlineEditor.prototype=domplate(Firebug.BaseEditor,{enterOnBlur:true,outerMargin:8,shadowExpand:7,tag:DIV({"class":"inlineEditor"},DIV({"class":"textEditorTop1"},DIV({"class":"textEditorTop2"})),DIV({"class":"textEditorInner1"},DIV({"class":"textEditorInner2"},INPUT({"class":"textEditorInner",type:"text",onkeypress:"$onKeyPress",onoverflow:"$onOverflow",oncontextmenu:"$onContextMenu"}))),DIV({"class":"textEditorBottom1"},DIV({"class":"textEditorBottom2"}))),inputTag:INPUT({"class":"textEditorInner",type:"text",onkeypress:"$onKeyPress",onoverflow:"$onOverflow"}),expanderTag:IMG({"class":"inlineExpander",src:"blank.gif"}),initialize:function(){this.fixedWidth=false;
...
if(isIE){this.input.style.top="-8px"
...
if(isIE){panel.panelNode.appendChild(this.box)
...
}}if(this.box.parentNode){try{this.input.setSelectionRange(0,0)
}catch(exc){}this.box.parentNode.removeChild(this.box)
}delete this.target;
delete this.panel
...
if(this.input.setSelectionRange){this.input.setSelectionRange(start,end)
...
if(textBox.setSelectionRange){textBox.setSelectionRange(originalOffset,originalOffset)
}this.reset();
return true
}else{this.reset();
return false
...
setTimeout(function(){if(selectMode){selectInputRange(textBox,offset,offsetEnd)
}else{selectInputRange(textBox,offsetEnd,offsetEnd)
}},0);
return true
}
};
...
var processStyleSheet=function(doc,styleSheet){var rules=isIE?styleSheet.rules:styleSheet.cssRules;
